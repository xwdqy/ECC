...
- ECC Tools billing/product readiness evidence, the local/internal announcement
  bearer-token path through exported env or ignored `--env-file`, usable
  Cloudflare/Wrangler auth for repeat KV evidence, and a live selected-target
  announcement gate pass before any native-payments announcement copy is
  published.
